...
## 一、真实标题区段（canonical headings）

### 1.1 三级小节

###### 1.1.1 深层六级标题

## 二、普通文本 hash marker 区段（plain text markers，不是标题）
...
## 四、canonical / plain 对照清单

| 行 | 类别 | 说明 |
|---|---|---|
| `# Document Diagnostics Heading Authority Test` | canonical H1 | 文档唯一一级标题 |
| `## 一、真实标题区段` | canonical H2 | 二级章节 |
| `### 1.1 三级小节` | canonical H3 | 三级小节 |
| `###### 1.1.1 深层六级标题` | canonical H6 | 六级深层标题 |
| `\#` | plain text marker | 转义后为普通文本，非 H1 |
| `\##` | plain text marker | 转义后为普通文本，非 H2 |
| `\####` | plain text marker | 转义后为普通文本，非 H4 |
| `## 三、真实跳级区段` | canonical H2 | 跳级起点（H3→H6 之后回退到 H2，正常） |
| `#### 3.1 四级小节` | canonical H4 | H2→H4 跳级（缺 H3） |
| `###### 3.1.1 六级子节` | canonical H6 | H4→H6 跳级（缺 H5） |
